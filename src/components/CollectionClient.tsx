              ...
              <Reveal key={p.id} delay={i * 100}>
                <Link href={`/product/${p.slug}`} className="product-card"
                  ...
                  <div className="product-card-img">
                    ...
                    {/* Default Image: BACK VIEW (back.png) */}
                    <Image src={p.images.back} alt={`${p.name} - Back View`} fill
                      sizes="(max-width:768px) 100vw, 50vw"
                      style={{ objectFit: "cover" }} />

                    {/* Hover Image: FRONT VIEW (front.png) */}
                    <div className="card-hover-img">
                      <Image src={p.images.front} alt={`${p.name} - Front View`} fill
                        sizes="(max-width:768px) 100vw, 50vw"
                        style={{ objectFit: "cover" }} />
                    </div>
                  </div>
