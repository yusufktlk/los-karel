...
export default function AccountClient() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "DELIVERED":
        return { label: isTR ? "TESLİM EDİLDİ" : "DELIVERED", color: "#4CAF50", bg: "rgba(76, 175, 80, 0.1)" };
      case "SHIPPED":
        return { label: isTR ? "KARGOLANDI" : "SHIPPED", color: "#2196F3", bg: "rgba(33, 150, 243, 0.1)" };
      case "PROCESSING":
        return { label: isTR ? "HAZIRLANIYOR" : "PROCESSING", color: "var(--clr-gold)", bg: "rgba(196, 168, 124, 0.1)" };
      default:
        return { label: isTR ? "BEKLEMEDE" : "PENDING", color: "#FF9800", bg: "rgba(255, 152, 0, 0.1)" };
    }
  };
    ...
    <>
      ...
      <section style={{ padding: "4rem 0 8rem", background: "var(--clr-bg)" }}>
        <div className="container" style={{ maxWidth: 880 }}>
          ...
          {/* Orders List */}
          {loading ? (
            <p style={{ color: "var(--clr-muted)", fontFamily: "var(--font-sans)" }}>{isTR ? "Siparişleriniz yükleniyor..." : "Loading your orders..."}</p>
                      ...
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "1rem", borderBottom: "1px solid var(--clr-border)", paddingBottom: "1.25rem", marginBottom: "1.75rem" }}>
                        <div>
                          <span style={{ fontFamily: "var(--font-title)", fontSize: "0.65rem", letterSpacing: "0.2em", color: "var(--clr-gold)", display: "block" }}>
                            {t("orderCodeLabel")}: #{o.id}
                          </span>
                          <p style={{ fontFamily: "var(--font-sans)", fontSize: "0.82rem", color: "var(--clr-muted)", marginTop: "0.25rem" }}>
                            {t("orderDateLabel")}: {new Date(o.createdAt).toLocaleString(isTR ? "tr-TR" : "en-US")}
                          </p>
                        </div>
                          ...
                          <div key={item.id} style={{ display: "flex", gap: "1.25rem", alignItems: "center" }}>
                            ...
                            <div style={{ flex: 1 }}>
                              <h4 style={{ fontFamily: "var(--font-title)", fontSize: "0.9rem", fontWeight: 500 }}>
                                {item.product?.name || "Edition Piece"}
                              </h4>
                              <p style={{ fontFamily: "var(--font-sans)", fontSize: "0.75rem", color: "var(--clr-muted)", marginTop: "0.2rem" }}>
                                {isTR ? "Beden" : "Size"}: <strong>{item.size}</strong> · {isTR ? "Adet" : "Qty"}: {item.quantity}
                              </p>
                            </div>
